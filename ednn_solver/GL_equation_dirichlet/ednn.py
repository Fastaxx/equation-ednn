...
class EvolutionalDNN:
    ...
    def eval_rhs(self,Input,Input_boundary,nbatch,w=None):
        ...
        dudt = self.rhs(self.output, Input, Input_boundary, self.eq_params)[0]
        dudt = np.concatenate([e.numpy().flatten() for e in dudt])

        dvdt = self.rhs(self.output, Input, Input_boundary, self.eq_params)[1]
    ...
    def output(self, X, X_boundary):
        ...
        #print(D1,D2,D3)
        XEaux = tf.concat([tf.reshape(X_boundary[:,0,0],[len(X),1,1]),tf.reshape(X[:,0],[len(X),1,1])],axis = 2)
        ...
        B_real = (tf.reshape((awest) / (awest + aeast),(-1,1)) * tf.reshape(U_boundary_pred[:,0],(-1,1))
            +  tf.reshape((aeast) / (aeast + awest),(-1,1)) * tf.reshape(U_boundary_pred[:,1],(-1,1)) )

        B_img = (tf.reshape((awest) / (awest + aeast),(-1,1)) * tf.reshape(V_boundary_pred[:,0],(-1,1))
            +  tf.reshape((aeast) / (aeast + awest),(-1,1)) * tf.reshape(V_boundary_pred[:,1],(-1,1)) )
 ...
 # For training real part of the EDNN at initial time on a batch of data. 
    @tf.function
    def training_step_real(self, X_batch,X_boundary_batch, Y_batch):
        with tf.GradientTape(persistent=True) as tape:
            Ypred = self.output(X_batch,X_boundary_batch)[0]
 ...
 # For training imaginary part of the EDNN at initial time on a batch of data. 
    @tf.function
    def training_step_img(self, X_batch,X_boundary_batch, Y_batch):
        with tf.GradientTape(persistent=True) as tape:
            Ypred = self.output(X_batch,X_boundary_batch)[1]
            aux = [tf.reduce_mean(tf.square(Ypred[i] - Y_batch[i,0])) for i in range(len(Ypred))]
            loss_data = tf.add_n(aux)
            loss = loss_data
        gradients_data = tape.gradient(loss_data,

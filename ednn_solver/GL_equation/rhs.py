...
#  RHS of GL equation. # main computational cost of EDNN involves automatic differentiation of the network outputs to evaluate the equation operator Nx(u)
@tf.function
def rhs_gl(output, coords, params):
    ...
    d2VdX2 = ddVdX[:,0]
    del tape2
    rhs_real = (-params[0]*dUdX+d2UdX2)+(2*params[1]*dVdX-params[2]*d2VdX2)+(params[3]-params[1]**2+(params[4]/2)*coords[:,0]**2)*U
    rhs_img =(-params[0]*dVdX+d2VdX2)+(-2*params[1]*dUdX+params[2]*d2UdX2)+(params[3]-params[1]**2+(params[4]/2)*coords[:,0]**2)*V
